...
uint8_t tag_data[8]={0};
//uint8_t first_tag_data[8]={0};
//uint8_t second_tag_data[8]={0};
uint8_t actual_tag_data[8]={0};

rfid_protect rfid_delay_protect={0};
...
Iso15693Anticollision(u08_t *mask, u08_t length)		// host command 0x14
{
  ...
  COUNT_VALUE = COUNT_1ms * 35;						// 20ms
	TIM3->ARR = ((u16)(COUNT_VALUE*125));
//  TIM3->APR = ((u16)(COUNT_VALUE*125) >>8);
//  TIM3->ARRL = COUNT_VALUE*125 -1;
...
//  delay_ms(50);
  ...
  case 0xFF:									// if recieved UID in buffer
    ...
    {	
      ...
      if(buf[10] != 0xe0)  //invalid data
        return;
      
      for(cnt1=0;cnt1<8;cnt1++)
        tag_data[cnt1]=buf[10-cnt1];
      
      for(cnt2=0;cnt2<8;cnt2++)
      {
        if(actual_tag_data[cnt2] != tag_data[cnt2])
        {
          for(cnt3=0;cnt3<8;cnt3++)
            actual_tag_data[cnt3] = tag_data[cnt3];
          
          if(rfid_delay_protect.RFID_DELAY_PROTECT_FLAG == 1)
          {
            rfid_delay_protect.rfid_delay_done = 0;
            rfid_delay_protect.delay_cnt = 0;
            rfid_delay_protect.delay_ms_cnt = 0;
          }
          read_same_tag_flag = 1;
          break;
        }
        else 
        {
          read_same_tag_flag = 0;    
          
          if(rfid_delay_protect.RFID_DELAY_PROTECT_FLAG == 1)
          {
            {
              for(cnt4=0;cnt4<8;cnt4++)
                actual_tag_data[cnt4] = tag_data[cnt4];
      ...
      if(read_same_tag_flag){
				//Ìí¼Ó¶Á¿¨³ÌÐò
      //SPM_MESSAGE_OUT(1,4,SEND_RFID_TAG_MESSAGES_H,0,&actual_tag_data[0]);
//      SPM_MESSAGE_OUT(1,4,SEND_RFID_TAG_MESSAGES_L,0,&actual_tag_data[4]);
      }
    }
    else
    {	    
