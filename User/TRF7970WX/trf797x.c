...
u8 InventoryRequest(unsigned char *ReadBuff);
...
//  mod_control[0] = CHIP_STATE_CONTROL;  //3v work
//  mod_control[1] = 0x01;
//  Trf797xWriteSingle(mod_control, 2);
  
  mod_control[0] = CHIP_STATE_CONTROL;
  mod_control[1] = CHIP_STATE_CONTROL;
  Trf797xReadSingle(&mod_control[1], 1);
  mod_control[1] |= BIT2;						// AGC ON
...
//	
//	
//	mod_control[0] = 0x00;						// AGC ON
//  mod_control[1] = 0x05;						// AGC ON 
//  Trf797xWriteSingle(mod_control, 2);
//	
//	mod_control[0] = 0x00;
//	Trf797xReadSingle(mod_control, 1);
...
{	
  ...
  {	// RX flag means that EOF has been recieved
    ...
    *irq_status = 0x7F & *irq_status;
    ...
    if(rxtx_state>10)                // 计数超过120则卷回，否则会内存溢出
		{
			rxtx_state = 1;
		}
  ...
  {									// RX active and 9 bytes allready in FIFO
    i_reg = 0x01;
    buf[rxtx_state] = FIFO;
    Trf797xReadCont(&buf[rxtx_state], 9);//86);	// read 86 bytes from FIFO
    rxtx_state = rxtx_state + 9;//86;
#if DBG
    UartPutChar('F');
#endif
...
*******************************************************************************/
//#pragma vector=0x07
//__interrupt void PORTC_IRQHandler(void)  // TRF7960 
...
void  GetUID (unsigned char *ReadBuff)
{
    ...
    command_1[0] = CHIP_STATE_CONTROL;   // 设置芯片状态控制寄存器，打开RF发射和接收 
    command_1[1] = 0x25;                 // RFID模块发射使能 接收使能
    command_1[2] = ISO_CONTROL;	         // 设置ISO控制寄存器，选择ISO15693 
    command_1[3] = 0x02;                 // ISO15693 高比特率，26.48kbps，一个副载波，4选1
    Trf797xWriteSingle(command_1, 4);    // 配置

    delay_ms(6);
   
    command_1[0] = CHIP_STATE_CONTROL;
    Trf797xReadSingle(command_1, 1);	              // 清除IRQs 
    command_1[0] = ISO_CONTROL;
    Trf797xReadSingle(command_1, 1);	              // 清除IRQs  
    ...
    command_1[0] = CHIP_STATE_CONTROL;	            // 关闭RF发射 
    command_1[1] = 0x01;
    Trf797xWriteSingle(command_1, 2);

    delay_ms(6);
//Trf797xResetIrqStatus();
    command_1[0] = 0x0C; //IRQStatus;
    Trf797xReadSingle(command_1, 1);	              // 清除 IRQs 
...
u8 InventoryRequest(unsigned char *ReadBuff)              
{
    ...
    unsigned int   size;                          // ?????????	
    unsigned char  NoSlots = 2;                  // ????,??1 ??(NoSlots = ??? + 1)
    unsigned int   n = 0;
	  u8 bbit; 
    ...
    {						 
        NoSlots = 17;		                        // 16 间隙 
        EnableSlotCounter();                 // 注释与否无影响
    }

    size = 3;                                   //?? = flag + command + lenght	
    ...
    command = 0x0C;                             // IRQStatus register;
    Trf797xReadSingle(&command, 1);
	
    SpiRawWrite(&buf[0], 8);	                  // 写 FIFO，向卡发送清查命令
...
//  ReadSingle(&command, 1);
    ...
    {							
      ...
      {								
        ...
        command = 0x0F;  //Reset;		// FIFO 在下个时隙前必须复位
        Trf797xDirectCommand(&command);
 
        if((NoSlots == 17) && (i < 16))         // 发送EOF(next slot)  
        {					
            command = 0x16;   //StopDecoders;
            Trf797xDirectCommand(&command);
            command = 0x17;   //RunDecoders;
            Trf797xDirectCommand(&command);
            command = 0x14;   //TransmitNextSlot;
            Trf797xDirectCommand(&command);
        }
...

{
    ...
    Trf797xReadSingle(&buff[1], 1);

    buff[1] |= 0x01;	

    Trf797xWriteSingle(&buff[0], 2);
